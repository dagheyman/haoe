...
int main(int argc, char *argv[]) {
    ...
    int_ptr = (int *) errorchecked_malloc(12); // allocating more heap memory

    *int_ptr = 31337; // put the value 31337 where int_ptr is pointing.
    printf("int_ptr (%p) ---> %d\n", int_ptr, *int_ptr);

    printf("\t [-] freeing char_ptr's heao memory..\n");
    free(char_ptr);

    printf("\t[+] allocating another 15 bytes for char_ptr\n");
    char_ptr = (char *) errorchecked_malloc(15); // moar heap memory
